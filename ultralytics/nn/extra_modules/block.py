...
__all__ = ['DyHeadBlock', 'DyHeadBlockWithDCNV3', 'Fusion', 'C2f_Faster', 'C3_Faster', 'C3_ODConv', 'C2f_ODConv', 'Partial_conv3', 'C2f_Faster_EMA', 'C3_Faster_EMA', 'C2f_DBB',
           ...
           'BiFusion', 'RepBlock', 'C3_EMBC', 'C2f_EMBC', 'SPPF_LSKA', 'CAM', 'SPPF_ELAN', 'ASPP', 'C3_DAttention', 'C2f_DAttention', 'C3_Parc', 'C2f_Parc', 'C3_DWR', 'C2f_DWR',
...
class SCConv(nn.Module):
    ...
    def __init__(self, c1, c2, s=1, d=1, g=1, pooling_r=4):
        ...
        # self.hsigmoid = nn.Hardswish()   # 添加hswish激活函数

    def forward(self, x):
        identity = x

        out = torch.sigmoid(torch.add(identity, F.interpolate(self.k2(x), identity.size()[2:]))) # sigmoid(identity + k2)
        # out = self.hsigmoid(torch.add(identity, F.interpolate(self.k2(x), identity.size()[2:]))) # sigmoid(identity + k2)
...
class SPPF_ELAN(nn.Module):
    """Spatial Pyramid Pooling - Fast"""

    def __init__(self, c1, c2, k=5):
        """
        Initializes the SPPF_ELAN layer with given input/output channels and kernel size.
        """
        super().__init__()
        c_ = c1 // 2  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)
